...
module Mux_8_1x64(//EDITED for 68 Bits  
	...
	generate
		for(k = 0; k < 4; k = k + 1)begin  : instantiate2
			...
			Mux_2_1_64Bit mux2_1 (.in0(in[output2]), .in1(in[output2 + 1'b1]), .out(stage3[k]), .sel(selector[0]));
	...
	generate
		for(l = 0; l < 2; l = l + 1)begin  : instantiate3
			...
			Mux_2_1_64Bit mux2_1 (.in0(stage3[output3]), .in1(stage3[output3 + 1'b1]), .out(stage4[l]), .sel(selector[1]));
	...
	Mux_2_1_64Bit mux2_1 (.in0(stage4[0]), .in1(stage4[1]), .out(out), .sel(selector[2]));
